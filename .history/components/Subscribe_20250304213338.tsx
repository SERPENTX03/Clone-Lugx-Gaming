const Subscribe = () => {
  return (
    <section className="sm:px-20 mt-32 w-full -z-20 flex justify-center">
      <div className="cat-bg relative lg:hidden block"></div>
      <div className="bg-gray-100 p-10 w-full">
        <h2 className="text-red-500 font-bold">OUR SHOP</h2>
        <div className="text-3xl font-bold">
          Go Pre-Order Buy & Get Best{" "}
          <span className="text-blue-500">Prices </span>
          For You!
        </div>
        <p>
          Lorem ipsum dolor consectetur adipiscing, sed do eiusmod tempor
          incididunt.
        </p>
        <button className="bg-red-500 text-white">SHOP NOW</button>
      </div>
      <div></div>
    </section>
  );
};
export default Subscribe;
